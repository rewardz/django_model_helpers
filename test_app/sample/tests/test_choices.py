...
def test_choices_functions():
    ...
    assert FRUITS.get_choice(1) == {
        "display": "strawberry", "value": 1, "extra_key": "extra_value", 'name': 'BERRY'
    }
    assert FRUITS.get_choice(2) == {"display": "Banana", "value": 2, "name": "BANANA"}
    assert FRUITS.get_choice(3) == {"display": "Water melon", "value": 3, "name": "WATER_MELON"}
...
def test_errors():
    with pytest.raises(ValueError):
        # error because value 3 is used by both CHEESE and WATER_MELON from parent class
        DuplicateFood()
    with pytest.raises(NotImplementedError):
        # old implementation is not supported anymore
        model_helpers.Choices([{"display": "strawberry", "value": 1, "extra_key": "extra_value"}])
