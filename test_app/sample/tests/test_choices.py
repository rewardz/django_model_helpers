...
def test_choices_order():
    ...
    assert choices() == [(3, "A_Choice_3"), (1, "Choice1"), (2, "Choice2")]
    # Order by "id"
    choices = model_helpers.Choices(
        [
            ("choice1", 1),
    ...
    assert choices() == [(1, "Choice1"), (2, "Choice2"), (3, "A_Choice_3")]
    # Disable ordering
    choices = model_helpers.Choices(
        [
            ("choice1", 1),
    ...
    assert choices() == [(1, "Choice1"), (3, "A_Choice_3"), (2, "Choice2")]
...
def test_concat_choices():
    ...
    assert choices2() == [(1, "X"), (2, "Y"), (5, "A"), (4, "B")]
...
def test_errors():
    ...
    with pytest.raises(TypeError):
        choices1.X = 7
    with pytest.raises(TypeError):
        setattr(choices1, "X", 7)  # noqa: WPS609, B010
    with pytest.raises(TypeError):
